...
export function GraphicDrawer({ g: initialGraphic, initialTab = "overview", hideTabs, onClose, onUpdate }: {
  ...
  const reviseBrief = async () => {
    ...
    try {
      ...
      updateCurrentGraphic(next);
      notify("rejected", `↩ Brief ถูกส่งกลับแก้: ${g.title}`, `ถึง ${g.requester} — ${comment} · โดย ${currentUser}`, "/my-tasks", "creative");
      setBriefComment("");
    } catch (error) {
  ...
  const requestFeedbackRevision = () => {
    ...
    notify("rejected", `✏️ งานกราฟฟิกถูกส่งกลับแก้: ${g.title}`, `${d.platform} — ${reason} · ถึง ${g.designer} · โดย ${currentUser}`, "/my-tasks", "creative");
    setFeedbackReason("");
    setTab("feedback");
  };
...
function DeliverablesEditor({ g, me, role, isRequester, onUpdate }: {
  ...
  const giveVerdict = (i: number, lens: ReviewLens, verdict: "pass" | "revise", note?: string) => {
    ...
    if (after.status === "Revision" && before.status !== "Revision") {
      ...
      notify("rejected", `✏️ งานกราฟฟิกถูกส่งกลับแก้: ${g.title}`, `${before.platform} — ${said} · ถึง ${g.designer} · โดย ${me}`, "/my-tasks", "creative");

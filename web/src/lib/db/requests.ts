...
type Row = {
  ...
  campaign: string | null; campaign_id: string | null;
  requester: string; approver: string; due: string; stage: string; priority: string;
  feedback?: RequestRow["feedback"];
};

const toReq = (r: Row): RequestRow => ({
  id: r.id, type: r.type, typeIcon: r.type_icon, title: r.title, b: r.brand,
  campaign: r.campaign ?? "—", campaignId: r.campaign_id ?? undefined,
...
export async function createRequest(r: RequestRow): Promise<void> {
  ...
  const { error } = await db.from("requests").upsert({
    ...
    campaign: r.campaign === "—" ? null : r.campaign, campaign_id: r.campaignId ?? null,

...
export interface ExpenseRow {
  vendor: string; category: string; b: BrandId; amount: number; vat: number;
  date: string; status: string; reimburseType?: string; wht?: number;
  /** Rate the withholding was taken at. 0 on rows predating the column — the
   *  voucher reconstructs a label from the amount rather than printing "3 %"
   *  over a deduction that was never 3%. */
  whtRate?: number;
}

...
| งาน | เข้าห้อง | ต่อยังไง |
|---|---|---|
| งานกราฟฟิก | `#05_marketing_graphic` | `SLACK_WEBHOOK_URL_GRAPHIC` |
| งาน Content (โพสต์ · แคปชั่น · publish) | `#07_marketing_content` | channel id ใน `org_settings` |
...
> **ห้อง Content มีตาข่ายรับไว้ห้องเดียวในระบบ:** ถ้าห้องมันใช้ไม่ได้ ข้อความจะตกไปเข้า `#05_marketing_graphic` เหมือนเดิมแทนที่จะเงียบ — ห้องอื่นไม่มี fallback ตามเดิม เพราะห้องที่ Content เคยอยู่ไม่ใช่ "ห้องผิด" ส่วนห้องอื่นเป็น

### ต่อห้องได้ 2 แบบ

1. **Incoming webhook** ใส่ใน env — แบบที่ 3 ห้องแรกใช้
2. **Channel id ใน `org_settings`** (key `slack_channel_ids` เก็บ `{"content":"C…"}`) แล้วโพสต์ด้วย bot token ตัวเดียวกับที่ส่ง DM — ห้อง Content ใช้แบบนี้

แบบที่ 2 มีดีตรง **ไม่มีอะไรที่เป็นความลับให้ต้องจัดการ** (channel id ไม่ใช่ credential) เพิ่มห้องใหม่ได้โดยไม่ต้องแตะ Vercel และไม่ต้อง redeploy · ข้อแลกคือบอทต้องอยู่ในห้องนั้น (ห้อง private ต้องเชิญเข้าไป) และต้องมี `SLACK_BOT_TOKEN` · ถ้าห้องไหนมีทั้งสองแบบ **webhook ชนะ**

หา channel id: เปิดห้องใน Slack → คลิกชื่อห้อง → บรรทัดล่างสุดของแท็บ About

### วิธีเอา webhook มา

1. https://api.slack.com/apps → แอป **Marketing OS**
2. เมนู **Incoming Webhooks** → เปิดสวิตช์ **On**
3. **Add New Webhook to Workspace** → เลือกห้อง → Allow
4. กด **Copy** ท้ายแต่ละแถว เอาไปใส่ env ตามตารางข้างบน

ห้องไหนยังไม่ได้ต่อ = งานประเภทนั้นเงียบ (ไม่ fallback ไปห้องอื่น เพราะส่งผิดห้องแย่กว่าไม่ส่ง) — Settings → Integrations จะบอกว่าห้องไหนยังขาด · ข้อยกเว้นเดียวคือ Content ที่ตกไป `#05` ตามที่เขียนไว้ข้างบน
